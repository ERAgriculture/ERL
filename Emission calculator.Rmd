...
3. **Estimated from nutritional composition**  
   Using the equation from Weiss et al. (2019):  
   \[
   \text{GE (Mkcal/kg)} = (0.056 × {CP}) + (0.094 × {EE}) + [0.042 × (100 − {CP} − {EE} − {Ash})]
   \]  
   
   Then converted to MJ/kg:  
   \[
   \text{GE (MJ/kg)} = \text{GE (kcal/kg)} x 4.184
   \]
...
4. **Estimated from digestible energy (DE)**  
   ...
   Using the equation from Weiss et al. (2019):
   \[
   \text{DE} = 1.044 + 0.0101 * {CP} + 0.0161 * {EE} - 0.0037 * {Ash} - 0.0023 * {NDF}
   \]
...
## 3) Complete the nutrition and digestibility papers
Not all ingredients and diets are there and we want to add them in order to be able to complete the data 
...
## infer diet nutrition from raw and feedipedia data 
```{r}
...
## infer digestibility form raw and feedipedia 
```{r}
...
## 14 estimate gross energy for ingredients from proximate nutritional composition
...
## 15 Convert digestible energy into gross energy 
...
convert feediepedia DE into GE 

all the feediepdia DE is in a % of GE so can't convert back 
...
##16  Calculation of CH4 emissions
...
- imorovment for Ym 
reorganize the method to be clear and intuitive 
