...
dry_name_match <- ingredients %>%
  ...
  select(B.Code, D.Item, Dry_by_name)


extra_dry <- ingredients %>%
  filter(
    D.Type %in% c("Non-ERA Feed", "Other Feed","Other Ingredients"),
...
dry_prediction_final <- full_join(dry_classified, dry_name_match,extra_dry, by = c("B.Code", "D.Item")) %>%
  mutate(
    Final_Prediction = coalesce(Dry_by_name, FALSE) |
                       coalesce(Dry_Prediction_Moisture, FALSE) |
                       coalesce(Dry_Prediction_DM, FALSE)
  ) %>%
  filter(Final_Prediction) %>%
  mutate(DC.Is.Dry = "Yes") %>%
  select(B.Code, D.Item, DC.Is.Dry)
...
diet_valid_units <- ingredients_cleaned %>%
  group_by(B.Code, A.Level.Name) %>%
  filter(n_distinct(D.Unit.Amount) == 1) %>%
  ungroup()
...
### 19.1) Merge yield and emission data 
