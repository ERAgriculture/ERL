...
add if treatments are control or not 

```{r}


# 1 ── Build a one-row-per-diet key from MT.Out ---------------------------
ctrl_key <- merged_metadata$MT.Out %>% 
  select(B.Code, A.Level.Name, T.Control) %>%      # keep only what we need
  distinct()                                       # drop any accidental duplicates
# If some diets still appear more than once (rare) choose one value:
#   distinct() above is usually enough; otherwise replace it with:
#   group_by(B.Code, A.Level.Name) %>% summarise(T.Control = first(T.Control), .groups = "drop")

# 2 ── Add T.Control to the ingredients table ----------------------------
ingredients <- ingredients %>% 
  left_join(ctrl_key, by = c("B.Code", "A.Level.Name"))
```
...
classify_diet_changes_detailed <- function(df) {
  df %>%
    group_by(B.Code) %>%
    group_split() %>%
    purrr::map_dfr(~{
      trial <- .x
      ctrl_tbl <- trial %>% filter(T.Control == "Yes")
      ctrl_items <- unique(ctrl_tbl$D.Item)

      trt_diets <- trial %>%
        filter(T.Control == "No") %>%
        group_by(A.Level.Name) %>%
        summarise(items = list(unique(D.Item)), .groups = "drop")

      if (length(ctrl_items) == 0 || nrow(trt_diets) == 0) return(NULL)

      trt_diets %>%
        rowwise() %>%
        mutate(
          added   = list(setdiff(items, ctrl_items)),
          removed = list(setdiff(ctrl_items, items)),
          practice_type = case_when(
            length(added) == length(removed) & length(added) > 0 ~ "substitution",
            length(added) > 0                                     ~ "addition",
            TRUE                                                  ~ NA_character_
          )
        ) %>%
        ungroup() %>%
        mutate(B.Code = unique(trial$B.Code)) %>%
        pivot_longer(cols = c(added, removed),
                     names_to = "change_type",
                     values_to = "D.Item") %>%
        unnest(D.Item) %>%
        left_join(
          df %>% select(B.Code, A.Level.Name, D.Item, D.Type) %>% distinct(),
          by = c("B.Code", "A.Level.Name", "D.Item")
        ) %>%
        select(B.Code, A.Level.Name, D.Item, D.Type, change_type, practice_type)
    })
}
...
# ── example call ─────────────────────────────────────────
practice <- classify_diet_changes_detailed(ingredients)
...
## add practice to ge combined to know if ingrediens were addded or substituted



```{r}
# Step 1: Aggregate practice type per diet
practice_summary <- practice %>%
  group_by(B.Code, A.Level.Name) %>%
  summarise(
    practice_type = first(na.omit(practice_type)),  # summarize per diet
    changed_types = paste(unique(D.Type), collapse = "; "),
    .groups = "drop"
  )

# Step 2: Merge into GE_combined
GE_combined <- GE_combined %>%
  left_join(practice_summary, by = c("B.Code", "D.Item" = "A.Level.Name"))
...
ei_prod <- GE_combined %>%                              # absolute CH4 table
  select(B.Code, A.Level.Name = D.Item,
         CH4_kg_year, P.Product,T.Control,practice_type,changed_types) %>%
...
```{r}
ei_prod_filtered <- ei_prod %>%
  filter(T.Control != "Yes") %>%   # Exclude control diets
  mutate(
    top_type = str_split(changed_types, ";\\s*") %>% map_chr(1)
  )

ggplot(ei_prod_filtered, aes(x = top_type, y = EI_CH4_prod, fill = practice_type)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.4) +
  ...
  scale_y_log10("Emission Intensity (kg CH₄ / kg product)",
                breaks = scales::log_breaks(),
                labels = scales::comma_format()) +
  labs(title = "Emission Intensity by Practice and Ingredient Type",
       x = "Ingredient Type") +
  theme_minimal(base_size = 13) +
  theme(legend.position = "bottom")
