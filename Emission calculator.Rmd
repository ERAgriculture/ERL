...
mt_cols <- c("B.Code", "A.Level.Name", "T.Animals","T.Name")
...
data_cols <- c("B.Code", "A.Level.Name","Out.Code.Joined", "ED.Intake.Item", "ED.Intake.Item.Raw", 
               "ED.Mean.T", "ED.Error","Out.Subind", "Out.Unit", "Out.WG.Start", "Out.WG.Unit","is_entire_diet","T.Animals")
...
#merge Mt and Data to have A.Level.Name in data 

camel$Data.Out<-merge(x=camel$Data.Out,
y= camel$MT.Out[,.(B.Code,T.Name,A.Level.Name)],by=c("B.Code","T.Name"),all.x=T,sort=F)
...
# Create herd_info with renamed T.Animals
herd_info <- camel$Herd.Out[, .(
  B.Code,
  A.Level.Name = Herd.Level.Name,
  Out.WG.Start = Herd.Start.Weight,
  Out.WG.Unit = Herd.Start.Weight.Unit,
  T.Animals = Herd.N  # Rename here
)]
...
data_camel <- camel$Data.Out[
  ...
  `:=`(
    ...
    T.Animals = i.T.Animals
  )
][
  , .(B.Code, A.Level.Name, ED.Intake.Item, ED.Intake.Item.Raw, 
      ED.Mean.T, ED.Error, Out.Unit, Out.Subind, Out.WG.Start, Out.WG.Unit, T.Animals)
][, Source := "camel"]
...
prod_camel <- camel$Herd.Out[, .(
  B.Code, 
  P.Product = V.Product, 
  Herd.Stage = Herd.Stage  # New column
)][, Source := "camel"]

# Step 2: Bind with livestock production data (which may not have Herd.Stage)
...
```{r}
#some papers also have ni units for feed intake 
#data and chekc in excel if unit is missing 
...
# )
# 
# rm(basal_expanded,basal_diets,non_basal_diets)
...
TO do : See BO1070 we ewant the full diet to be included form feed intake which is not the case now see feed intake data the goal would be to add rows in ingredient to add info at the entire diet level have a close look to make it the smae format as entire diets alreayd in ingredients 
...
try to take out the diet for which grazing is involved d type = grazing and see the loss of papers 
...
url <- "https://raw.githubusercontent.com/Mlolita26/ERA_Docs/main/era_master_sheet(4).xlsx"
...
mutate(
  ...
  DC.Value = case_when(
    ...
    DC.Variable == "GE" ~ case_when(
      ...
      DC.Unit == "kcal/100g" ~ DC.Value * 0.4184,
      DC.Unit == "cal/g" ~ DC.Value * 0.000004184,
      DC.Unit == "mcal/kg" ~ DC.Value * 4.184,
      DC.Unit %in% c("kj/kg", "kj/kg dm") ~ DC.Value / 1000,
      DC.Unit %in% c("kj/g") ~ DC.Value,  # Already MJ/kg if value is per g * 1000
      DC.Unit %in% c("kj/100g", "kj/100g dm", "kj/100 g dm") ~ DC.Value * 0.01,
      DC.Unit == "mj/100g" ~ DC.Value * 10,
      DC.Unit == "j/mg" ~ DC.Value / 1e6,
      TRUE ~ NA_real_  # unknown unit
    ),

    # --- All other nutrient variables: convert to g/kg ---
    TRUE ~ case_when(
...
## 9) Relative % of each ingredient in the diet
...
### 10.1 Estimation of Enteric Methane Emissions Using IPCC Tier 2
...
### 10.3. Methane Emissions Calculation (Tier 2)
...
### 10.4 Data Requirements and Supporting Tables
...
## 11) Gross energy data 

## 11.1) Gross energy harmonization
...
## 13) Infer gross energy from data 
...
## 14) Estimate Digestible energy from proximate nutritional composition
...
### 16.2) Reconstruct Gross Energy at the diet level from ingredient data 
...
### 16.3) Gross energy at the entire diet level 
```{r,message=FALSE,warning=FALSE}
...
### 16.4) Merge GE data with feed intake 
...
## 17) Link emissions to productivity data 
...
### 17.1) Merge yield and emission data 
...
forage_types    <- c("Forage Trees", "Herbaceous Fodders", "Agroforestry Fodders")
forage_keywords <- c("hay", "straw", "stover","silage","grass")


forage_summary <- ingredients %>% 
  
  # 1. Harmonise units and flag forage ------------------------------
  mutate(
    D.Amount_kg = if_else(D.Unit.Amount == "g", D.Amount / 1000, D.Amount),
    
    is_forage   =  D.Type %in% forage_types  |                        # exact match in D.Type
                   str_detect(str_to_lower(D.Item),
                              str_c(forage_keywords, collapse = "|")) # keyword in D.Item
  ) %>% 
  
  # 2. Keep diets that use one single mass‑unit ----------------------
  group_by(B.Code, A.Level.Name) %>% 
  filter(n_distinct(D.Unit.Amount, na.rm = TRUE) == 1) %>% 
  
  # 3. Compute forage proportion ------------------------------------
  summarise(
    forage_prop   = sum(D.Amount_kg[is_forage]) / sum(D.Amount_kg),
    n_ingredients = n(),
    .groups       = "drop"
  )


# 4. Attach CH₄ data and keep diets that appear ≥ 2 times ----------
forage_ch4 <- forage_summary %>% 
  inner_join(
    GE_combined %>% select(B.Code, D.Item, CH4_kg_year),
    by = c("B.Code", "A.Level.Name" = "D.Item")
  ) %>% 
  left_join(
    merged_metadata$Prod.Out %>% select(B.Code, P.Product),   #  ⇦ add this line
    by = "B.Code"
  ) %>% 
  group_by(B.Code) %>% 
  filter(n() >= 2) %>% 
...
# 5A. *Relative* change in CH₄ within each diet --------------------
relative_change <- forage_ch4 %>% 
  group_by(B.Code) %>% 
  summarise(
    rel_decrease_ch4 = 100 * diff(range(CH4_kg_year)) / max(CH4_kg_year),
    min_forage       = min(forage_prop),
    max_forage       = max(forage_prop),
    P.Product        = first(P.Product),
    .groups          = "drop"
  )

# 5B. *Slope* (ΔCH₄ / Δforage) within each diet -------------------
#   – sometimes more interpretable than % change
slopes <- forage_ch4 %>% 
  group_by(B.Code) %>% 
  summarise(
    slope_ch4 = {
      ok <- complete.cases(forage_prop, CH4_kg_year)
      if (sum(ok) >= 2) coef(lm(CH4_kg_year[ok] ~ forage_prop[ok]))[2] else NA_real_
    },
    r2 = {
      ok <- complete.cases(forage_prop, CH4_kg_year)
      if (sum(ok) >= 2) summary(lm(CH4_kg_year[ok] ~ forage_prop[ok]))$r.squared else NA_real_
    },
    P.Product = first(P.Product),
    .groups   = "drop"
  )
...
ggplot(slopes,
       aes(P.Product, slope_ch4, fill = P.Product)) +
  geom_hline(yintercept = 0, linetype = 2) +
  geom_boxplot(alpha = 0.4, outlier.shape = NA) +
  geom_jitter(width = 0.15, alpha = 0.7, size = 2) +
  labs(
    title = "Within‑diet slope of CH₄ vs. forage proportion",
    y = "Slope (kg CH₄ per unit forage proportion)"
  ) +
...
```{r}
########################################################
## % concentrate (strict rule) vs. CH₄ g kg⁻¹ DMI     ##
########################################################

## 1)  Tag forage and concentrate ------------------------------------------------
forage_types    <- c("Forage Trees", "Herbaceous Fodders",
                     "Agroforestry Fodders", "Pasture", "Crop Byproduct")
forage_keywords <- c("hay", "straw", "stover", "silage", "grass")

ingredients_fc <- ingredients %>%
  mutate(
    name_lc        = str_to_lower(D.Item),
    
    is_forage      = D.Type %in% forage_types |
                     str_detect(name_lc, str_c(forage_keywords, collapse = "|")),
    
    is_concentrate = str_detect(name_lc, "concentrate") |
                     D.Type == "Concentrate",
    
    category       = case_when(
                       is_forage      ~ "Forage",
                       is_concentrate ~ "Concentrate",
                       TRUE           ~ "Other"
                     )
  )

## 2)  Compute % concentrate per diet ------------------------------------------
fc_ratio <- ingredients_fc %>%
  mutate(D.Amount_kg = if_else(D.Unit.Amount == "g", D.Amount / 1000, D.Amount)) %>%
  group_by(B.Code, A.Level.Name) %>%
  summarise(
    kg_conc   = sum(D.Amount_kg[category == "Concentrate"], na.rm = TRUE),
    kg_total  = sum(D.Amount_kg,                         na.rm = TRUE),
    pct_conc  = if_else(kg_total > 0, kg_conc / kg_total * 100, NA_real_),
    .groups   = "drop"
  )

## 3)  Bring in intake (kg DMI d‑¹) and CH₄ (kg y‑¹) ---------------------------
intake_tbl <- feed_intake_diet %>%
  select(B.Code, A.Level.Name, DMI_kg_d = ED.Mean.T)

ch4_tbl <- GE_combined %>%
  rename(A.Level.Name = D.Item) %>%
  select(B.Code, A.Level.Name, CH4_kg_year)

fc_full <- fc_ratio %>%
  left_join(intake_tbl, by = c("B.Code", "A.Level.Name")) %>%
  left_join(ch4_tbl,   by = c("B.Code", "A.Level.Name")) %>%
  left_join(
    merged_metadata$Prod.Out %>% select(B.Code, P.Product),
    by = "B.Code"
  ) %>%
  mutate(
    CH4_g_kgDMI = CH4_kg_year * 1000 / (DMI_kg_d * 365)
  ) %>%
  filter(!is.na(pct_conc), !is.na(CH4_g_kgDMI), !is.na(P.Product))

## 4)  Plot % concentrate vs CH₄ intensity -------------------------------------
ggplot(fc_full,
       aes(x = pct_conc, y = CH4_g_kgDMI, colour = P.Product)) +
  geom_point(alpha = 0.7, size = 2.3) +
  geom_smooth(method = "lm", se = TRUE, linetype = "dashed") +
  labs(
    title  = "Concentrate proportion vs. CH₄ intensity",
    x      = "% concentrate in diet (DM basis)",
    y      = "CH₄ (g kg⁻¹ DMI)",
    colour = "Species"
  ) +
  theme_minimal(base_size = 13) +
...
## 5)  Species‑level slopes -----------------------------------------------------
fc_slopes <- fc_full %>%
  group_by(P.Product) %>%
  summarise(
    slope_gCH4_per_pctC = coef(lm(CH4_g_kgDMI ~ pct_conc))[2],
    r2                  = summary(lm(CH4_g_kgDMI ~ pct_conc))$r.squared,
    n_diets             = n(),
    .groups = "drop"
  )
