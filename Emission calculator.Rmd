...
data_cols <- c("B.Code", "A.Level.Name","Out.Code.Joined", "ED.Intake.Item", "ED.Intake.Item.Raw", 
               "ED.Mean.T", "ED.Error","Out.Subind", "Out.Unit", "Out.WG.Start", "Out.WG.Unit","is_entire_diet")
...
#merge Mt and Data to ave A.Level.Name in data 
...
data_camel <- camel$Data.Out[
  , .(B.Code, ED.Intake.Item, 
      ...
      A.Level.Name,  # Temporarily keep this as-is for now
...
feed_intake <- merged_metadata$Data.Out %>%
  select(B.Code, A.Level.Name, ED.Intake.Item, ED.Intake.Item.Raw,
         ED.Mean.T, ED.Error, Out.Unit, Out.Subind,is_entire_diet) %>%
  filter(Out.Subind == "Feed Intake") %>%
...
GE_diet_reconstructed <- GE_from_ingredients %>%
  left_join(total_amounts %>% select(B.Code, A.Level.Name, Total_Amount, D.Unit.Amount),
            by = c("B.Code", "A.Level.Name")) %>%
  ...
  mutate(
    ...
    # Assign Ym using purpose and forage info
    Ym = case_when(
...
forage_proportions <- diet_valid_units %>%
  left_join(merged_metadata$Prod.Out, by = "B.Code") %>%
  filter(D.Type != "Entire Diet") %>%
  group_by(B.Code, A.Level.Name) %>%
  summarise(
    ...
    total_forage = ifelse(any_na, NA_real_, sum(D.Amount[D.Type %in% forage_types], na.rm = TRUE)),
    n_ingredients = n(),
    is_forage = all(D.Type %in% forage_types),
    D.Type.first = D.Type[1],
    D.Unit.Amount = first(D.Unit.Amount),
...
# Step 1: Join forage proportion with CH₄ emissions and species info
forage_emission_species <- forage_proportions %>%
  left_join(GE_combined %>% select(B.Code, D.Item, CH4_kg_year),
            by = c("B.Code", "A.Level.Name" = "D.Item")) %>%
  filter(!is.na(forage_prop), !is.na(CH4_kg_year)) %>%
  group_by(B.Code) %>%
  filter(n() > 1) %>%  # Keep only B.Codes with more than one diet
  ungroup()

# Step 2: Calculate relative *decrease* in CH₄ emissions when forage proportion is reduced
rel_decrease <- forage_emission_species %>%
  group_by(B.Code) %>%
  arrange(forage_prop, .by_group = TRUE) %>%
  summarise(
    min_forage = first(forage_prop),
    max_forage = last(forage_prop),
    min_CH4 = first(CH4_kg_year),
    max_CH4 = last(CH4_kg_year),
    P.Product = first(P.Product),
    .groups = "drop"
  ) %>%
  filter(max_CH4 > 0) %>%
  mutate(
    rel_decrease_CH4 = (max_CH4 - min_CH4) / max_CH4 * 100  # % decrease in CH₄ when forage is reduced
  )

# Step 3: Plot relative CH₄ decrease by species
ggplot(rel_decrease, aes(x = P.Product, y = rel_decrease_CH4, fill = P.Product)) +
  geom_boxplot(alpha = 0.5) +
  geom_jitter(width = 0.2, alpha = 0.7, size = 2) +
  labs(
    title = "Relative Decrease in CH4 Emissions with Lower Forage Proportion",
    x = "Livestock Species",
    y = "Relative Decrease in CH4 (%)"
  ) +
  theme_minimal() +
  theme(legend.position = "none")
...
```{r}
# Step 1: Join forage proportions with CH₄ emissions and species info
forage_emission_continuous <- forage_proportions %>%
  left_join(
    GE_combined %>% select(B.Code, D.Item, CH4_kg_year),
    by = c("B.Code", "A.Level.Name" = "D.Item")
  ) %>%
  filter(!is.na(forage_prop), !is.na(CH4_kg_year), !is.na(P.Product))

# Step 2: Plot the continuous relationship with smoothing
ggplot(forage_emission_continuous, aes(x = forage_prop, y = CH4_kg_year, color = P.Product)) +
  geom_point(alpha = 0.6, size = 2.5) +
  geom_smooth(method = "lm", se = TRUE, linetype = "dashed", size = 1) +
  labs(
    title = "Relationship Between Forage Proportion and CH₄ Emissions",
    x = "Forage Proportion in Diet",
    y = "Annual CH₄ Emissions (kg)",
    color = "Species"
  ) +
  theme_minimal() +
  theme(legend.position = "bottom")
