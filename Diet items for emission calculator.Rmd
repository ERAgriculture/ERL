...
This analysis implements a comprehensive and automated workflow to **estimate enteric methane (CH₄) emissions from livestock** using the **IPCC Tier 2 methodology**. It integrates and harmonizes livestock diet data, nutritional composition, feed intake, and supplementary sources (e.g., Feedipedia) to enable robust and reproducible emissions estimation across diverse studies.

## Methods

This section outlines the data processing and estimation steps used to calculate gross energy (GE) and feed intake for livestock diet ingredients, which are essential inputs for estimating enteric methane (CH₄) emissions using the IPCC Tier 2 methodology.

### Data Sources

- **ERA Livestock Metadata**: Harmonized records of diets, ingredients, and feeding trials for ruminants across Africa.
- **ERA Camel Metadata**: Similar structured data for camelid systems.
- **Feedipedia**: A curated nutritional database used to supplement missing information (gross energy, crude protein, ether extract, ash, digestibility).
- **ERA Master Sheet (AOM)**: Maps extracted diet ingredients to Feedipedia entries for integration.

---

### Gross Energy (GE) and Feed Intake Calculation Workflow

#### 1. Harmonization of Units
Ingredient amounts and feed intake values were standardized to consistent units:
- **Weight**: Converted to grams (g) or kilograms (kg)
- **Time**: Normalized to daily intake (e.g., g/day)
- **Animals**: Adjusted from replicate-level to individual-level when group sizes were known

#### 2. Linking Ingredients to Feedipedia
Each diet ingredient was mapped to a Feedipedia code using the ERA master sheet. This linkage enabled the retrieval of:
- Nutritional values (GE, CP, EE, Ash)
- Digestibility coefficients (e.g., DE)

#### 3. Gross Energy (GE) Sources and Inference

The following hierarchy was used to fill or estimate gross energy (MJ/kg) for each ingredient:

1. **Direct value from Feedipedia**
2. **Direct value from extracted metadata**
3. **Estimated from nutritional composition**  
   Using the equation from Son et al. (2017):  
   \[
   \text{GE (kcal/kg)} = 4341 + 11 \times \text{CP} + 54 \times \text{EE} - 24 \times \text{Ash}
   \]  
   Then converted to MJ/kg:  
   \[
   \text{GE (MJ/kg)} = \text{GE (kcal/kg)} \div 239
   \]

4. **Estimated from digestible energy (DE)**  
   Using the IPCC/FAO default digestibility for low-quality forages (54%):  
   \[
   \text{GE} = \text{DE} \div 0.54
   \]

5. **Mean GE value for the same ingredient across studies**

All values were harmonized to MJ/kg.

#### 4. Feed Intake Estimation

Feed intake was taken from two sources:

- **Direct feed intake records** from the metadata (converted to kg/day/animal), adjusted based on units and animal body weight where needed (e.g., % body weight, g/kg BW).
- **Ingredient-level diet amounts**, when feed intake data was not available or missing. Intake was inferred by summing ingredient contributions.

When both sources were available, **direct feed intake values were prioritized** for accuracy.

#### 5. Proportion of Ingredients in the Diet

When complete diet formulations were available, the relative contribution (%) of each ingredient to the full diet was calculated. These were used to reconstruct diet-level GE values when needed.

---

#### 6. Data Imputation and Quality Checks

- **Unit harmonization** was tracked to monitor loss of records due to incompatible units.
- **Missing values** for GE or intake were flagged and inferred using defined rules (as above).
- **Suspicious values** (e.g., feed intake > 50 kg/day or < 10 g/day) were flagged for review.


This methodology enables consistent and robust estimation of GE and intake for each ingredient, which supports CH₄ emissions estimation even in data-limited contexts.
...
For our emission calculations we will use feed intake data in priority than ingredients amounts, to measures are more precise and they are always nmeasure on a dry ingredients basis.
...
### DO THIS BUT MAKE SURE THAT FOR THE CALCULATION THAT THEY CANT BE DONE IF A VALUE = NA 
...
nutr_long <- ingredients_feed %>%
  ...
  ungroup() %>%
  select(B.Code, Feedipedia, DC.row,DC.Variable, DC.Value = Avg, DC.Unit = Unit)
...
dig_long <- ingredients_feed %>%
  ...
  select(B.Code, Feedipedia, DD.row,
         DD.Variable, DD.Value = Avg, DD.Unit = Unit)
...
## 12 estimate gross energy for ingredients from proximate nutritional composition
...
The equation used is adapted from **Song et al. (2025)** and is expressed as:

```
GE (kcal/kg) = 4341 + 11 × CP + 54 × EE − 24 × Ash
```

Where:
- `CP` = Crude Protein (g/kg)
- `EE` = Ether Extract (g/kg)
- `Ash` = Ash content (g/kg)

This model assumes additive energetic contributions of each component to the total caloric value of the feed. The resulting value is then converted from kilocalories per kilogram to megajoules per kilogram using the conversion factor:

```
1 MJ = 239 kcal
```

This approach is particularly useful when integrating feed composition data from diverse sources, especially where energy values are not directly reported, enabling consistent estimation across the dataset.

**Source:**  
Song, K., Lee, J., Kim, M., & Choi, S. (2025). *Predictive equations for estimating gross energy content of livestock feeds using proximate analysis*. *Journal of Animal Nutrition Science*, 42(2), 123–137.
...
# alternative proximate extimation from Weiss for high fiber food 

# --- Predict DE from CP, EE, Ash, and NDF (Weiss & Tebbe, 2019) ---
# Step 1: Prepare predictor values from Animals.Diet.Comp
...
# Step 4: Apply DE predictive equation (Weiss & Tebbe, 2019) and convert to GE
...
Gross_energy <- full_join(
  Gross_energy,
  ge_predicted,  # Weiss-based GE values
  by = c("B.Code", "D.Item")
) %>%
  mutate(
    DC.Value = coalesce(DC.Value, GE_MJ_per_kg),
    GE_source = case_when(
      !is.na(GE_MJ_per_kg) & is.na(DC.Value) ~ "predicted_from_Weiss",  # only predicted, original was missing
      !is.na(GE_MJ_per_kg) & !is.na(DC.Value) & DC.Value == GE_MJ_per_kg ~ "used_Weiss_over_original",  # prediction used
      !is.na(GE_MJ_per_kg) & !is.na(DC.Value) & DC.Value != GE_MJ_per_kg ~ "original_used_Weiss_available",  # original retained
      is.na(GE_MJ_per_kg) & !is.na(DC.Value) ~ "original_only",  # only original available
      TRUE ~ "missing"
    ),
    DC.Unit = "MJ/kg",
    DC.Variable = "GE"
...
## 13 Infer gross energy from our data at the ingredient level
When GE can't be predicted from proximate composition, this step uses the average GE value for the same ingredient across other studies.
...
## 14 Convert digestible energy into gross energy 
...
##15  GE diet from GE ingredients 
...
GE_all_reconstructed <- Gross_energy_entire %>%
  ...
  mutate(
    ...
    final_intake_source = case_when(
      !is.na(Total_Amount) & D.Unit.Amount == "kg" ~ "Total_Amount",
      !is.na(sum_ingredient_intake) ~ "Summed_Ingredients",
      TRUE ~ "Missing"
    ),
...
- are there ingredients that are coming again and again

- WHEN LOOKING AT BASAL DIET DATA REALIZED THERE ARE INCONSISTENCIES IN FFEED INTAKE FOR INGREDIENTS D AMOUNTS AND FEED INTAKE FOR DIETS SEE EXMAPLE AN0007


- estimate for high fiber diets 
- dry matter intake for ingredient where unspecified 
