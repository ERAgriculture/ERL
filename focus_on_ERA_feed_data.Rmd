---
title: "Focus on ERA Animal Feed Data"
...
## 4) Extract Animal Diet Data from livestock_data

The ERA dataset is divided into multiple tables. We focus on those describing animal diets, nutritional composition, digestibility, and bibliographic references.

### 4.1) Animal Diet Overview (`master_codes$Animals.Out`)
...
## 5) Nutritional Composition of Feeds (`mastercodes$Animals.Diet.Comp`)
...
### 5.1) Clean and Filter the Nutrition Data
...
DT::datatable(diet_nutrition[!grepl("No Match", D.Item.AOM)], options = list(pageLength = 5))
```
...
## 6) Digestibility of Feeds (`master_codes$Animals.Diet.Digest`)
...
### 6.1) Clean and Filter Digestibility Data
...
cat("Most digestibility records in ERA are for entire diets, accounting for",
    round(100 * (full_length - sub_length) / full_length, 1), "% of records.\n")
...
DT::datatable(diet_digest[!grepl("No Match", D.Item.AOM)], options = list(pageLength = 5))
```
