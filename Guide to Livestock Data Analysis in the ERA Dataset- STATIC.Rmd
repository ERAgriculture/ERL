---
title: "Evidence for Resilient Livestock: Data Exploration- STATIC"
author:
  - name: "Namita Joshi"
...
# Filter the data
filtered_data_prac <- eragri::PracticeCodes %>%
...
DT::datatable(
  head(eragri::OutcomeCodes),  # Display the first few rows of PracticeCodes
  options = list(
    scrollY = "400px",    # Set vertical scroll height
    scrollX = TRUE,       # Enable horizontal scrolling
    pageLength = 10,      # Display 10 rows per page
...
# Filter the data
filtered_data_eu <- eragri::EUCodes %>%
...
This section retrieves the most recent version of the livestock dataset from an S3 bucket. It lists the available files, downloads the required .RData file, and loads it into R using miceadds::load.Rdata2. The objective is to prepare the dataset (livestock_data) for analysis.
...
```{r calculating meanT and variance,message=F,echo=T,collapse=FALSE}

#################TESTING

data_variance_TEST <- data %>%
  group_by(Code, DOI, Country, Site.ID, CID, C.Descrip, M.Year, Units, MeanC, 
           Out.SubInd, PrName, Product.Simple) %>%  # Removed Tree.Feed from group_by
  mutate(
    VarianceT = var(MeanT, na.rm = TRUE)  # Compute variance of MeanT within each group
  ) %>%
  ungroup()

# Now summarize with correct variance
av_meanT_TEST <- data_variance_TEST %>%
  group_by(Code, DOI, Country, Site.ID, CID, C.Descrip, M.Year, Units, MeanC, 
           Out.SubInd, PrName, Product.Simple) %>%  # Still not grouping by Tree.Feed
  summarise(
    TID = paste(unique(TID), collapse = ".."),
    T.Descrip = paste(unique(T.Descrip), collapse = ".."),
    Tree.Feed = paste(unique(Tree.Feed), collapse = ".."),  # Aggregate Tree.Feed
    MeanT = mean(MeanT, na.rm = TRUE),  # Take mean of MeanT
    VarianceT = mean(VarianceT, na.rm = TRUE)  # Take mean of precomputed variance
  ) %>%
  ungroup()

######################################################################################
##Adding DM and CP to larger data

data_1<- data

# Step 1: Filter digest to only include "DM" and "CP"
digest_filtered <- digest %>%
  filter(DD.Variable %in% c("DM", "CP"))

# Step 2: Merge control values using CID (only for DM and CP)
data_1 <- data_1 %>%
  left_join(
    digest_filtered %>% select(B.Code, D.Item, DD.Value, DD.Unit, DD.Variable),
    by = c("C.Descrip" = "D.Item", "Code" = "B.Code")
  ) %>%
  rename(DD.Value_Control = DD.Value,  # Rename to indicate control
         DD.Unit_Control = DD.Unit, 
         DD.Variable_Control = DD.Variable)  # Keep track of Variable type

# Step 3: Merge treatment values using TID (only for DM and CP), ensuring a match on DD.Variable
data_1 <- data_1 %>%
  left_join(
    digest_filtered %>% select(B.Code,D.Item, DD.Value, DD.Unit, DD.Variable),
    by = c("T.Descrip" = "D.Item", "DD.Variable_Control" = "DD.Variable","Code" = "B.Code")  # Ensures DM matches DM, CP matches CP
  ) %>%
  rename(DD.Value_trt = DD.Value,  # Rename to indicate treatment
         DD.Unit_trt = DD.Unit, 
         DD.Variable_trt = DD.Variable_Control)  # Keep consistent variable naming

check<-data_1 %>%
  select(Code, DD.Value_Control,DD.Unit_Control, DD.Unit_trt,DD.Value_trt,DD.Unit_trt,T.Descrip,C.Descrip, DD.Variable_trt)

#############################################################################################
##Averaging and calculating 

# Step 1: Calculate Variance and Summarize in `data_variance`
data_variance <- data_1 %>%
  group_by(Code, DOI, Country, Site.ID, CID, C.Descrip, M.Year, Units, MeanC, 
           Out.SubInd, PrName, Product.Simple, DD.Variable_trt,
           DD.Unit_Control, DD.Unit_trt, DD.Value_Control) %>%  # Include DD.Value_Control
  mutate(
    VarianceT = var(MeanT, na.rm = TRUE)  # Compute variance of MeanT within each group
  ) %>%
  ungroup()
...
av_meanT <- data_variance %>%
  ...
  summarise(
    ...
    Tree.Feed = paste(unique(Tree.Feed), collapse = ".."),  # Aggregate Tree.Feed
    MeanT = mean(MeanT, na.rm = TRUE),  # Take mean of MeanT
    VarianceT = mean(VarianceT, na.rm = TRUE),  # Take MEAN of precomputed variance (NOT var again!)
    DD.Value_trt = mean(DD.Value_trt, na.rm = TRUE)  # Keep DD.Value_trt as mean
  ) %>%
  ungroup()

# Display the first few rows of the new dataset
head(av_meanT)
write.csv(av_meanT,"diet_data/av_meanT_DM_CP.csv")
...
# -----------------------------------
# Step 1: Prepare Digest Data (DM & CP only)
# -----------------------------------
digest_filtered <- digest %>%
  filter(DD.Variable %in% c("DM", "CP"))

# -----------------------------------
# Step 2: Merge Digest Data into Main Data (both Control and Treatment)
# -----------------------------------
data_with_digest <- data %>%
  # Add Control Values (by Code and C.Descrip)
  left_join(
    digest_filtered %>% select(B.Code, D.Item, DD.Value, DD.Unit, DD.Variable),
    by = c("Code" = "B.Code", "C.Descrip" = "D.Item")
  ) %>%
  rename(DD.Value_Control = DD.Value,
         DD.Unit_Control = DD.Unit,
         DD.Variable = DD.Variable) %>%
  
  # Add Treatment Values (by Code and T.Descrip, and ensure variable match)
  left_join(
    digest_filtered %>% select(B.Code, D.Item, DD.Value, DD.Unit, DD.Variable),
    by = c("Code" = "B.Code", "T.Descrip" = "D.Item", "DD.Variable" = "DD.Variable")
  ) %>%
  rename(DD.Value_trt = DD.Value,
         DD.Unit_trt = DD.Unit)

# -----------------------------------
# Step 3: Calculate Variance Within Groups
# -----------------------------------
data_variance <- data_with_digest %>%
  group_by(Code, DOI, Country, Site.ID, CID, C.Descrip, M.Year, Units, MeanC, 
           Out.SubInd, PrName, Product.Simple, DD.Variable,
           DD.Unit_Control, DD.Unit_trt, DD.Value_Control) %>%
  mutate(
    VarianceT = var(MeanT, na.rm = TRUE)
  ) %>%
  ungroup()

# -----------------------------------
# Step 4: Summarize to Get Grouped Means
# -----------------------------------
av_meanT <- data_variance %>%
  group_by(Code, DOI, Country, Site.ID, CID, C.Descrip, M.Year, Units, MeanC, 
           Out.SubInd, PrName, Product.Simple, DD.Variable,
  ...
  summarise(
    TID = paste(unique(TID), collapse = ".."),
    T.Descrip = paste(unique(T.Descrip), collapse = ".."),
    Tree.Feed = paste(unique(Tree.Feed), collapse = ".."),
    MeanT = mean(MeanT, na.rm = TRUE),
    VarianceT = mean(VarianceT, na.rm = TRUE),
    DD.Value_trt = mean(DD.Value_trt, na.rm = TRUE)
  ) %>%
  ungroup()
...
# -----------------------------------
write.csv(av_meanT, "diet_data/av_meanT_DM_CP.csv", row.names = FALSE)
...
```{r exploring the diet ingredients, nutrition, digestibility tables, echo=TRUE}
  ...
  diet_summary<-livestock_data$Animals.Out
  
    diet_ingredients<-livestock_data$Animals.Diet
  head(diet_ingredients)
  
  diet_nutrition<-livestock_data$Animals.Diet.Comp
  head(diet_nutrition)
  
  diet_digestibility<-livestock_data$Animals.Diet.Digest
...
feed_intake <- livestock_data$Data.Out[
...
This script performs a detailed analysis by merging data on livestock feed intake, weight gain, and nutritional composition to create a comprehensive dataset. First, it renames the ED.Intake.Item column to D.Item_raw in the intake_WG dataset for consistency. Then, it filters the livestock_data$Animals.Diet.Comp dataset to exclude rows with missing values in the DC.Value column, ensuring only complete nutritional data is used. Relevant columns (B.Code, DC.Value, DC.Unit, DC.Variable, D.Item_raw) are selected from this filtered dataset. The script merges the nutritional data with the feed intake and weight gain dataset (intake_WG) using an inner join, retaining only rows with matching values in D.Item_raw and B.Code

```{r adding nutrition, echo=TRUE, message=FALSE, warning=FALSE}
  nutrition <- livestock_data$Animals.Diet.Comp %>% 
    filter(!is.na(DC.Value)) %>%  # Omit rows with NA in DC.Value
    select(B.Code, DC.Value, DC.Unit, DC.Variable, D.Item_raw)

  nutrition<- nutrition %>%
      rename(`D.Item.raw`= D.Item_raw)
...
```
```{r calculating feed dosage, echo=TRUE, message=FALSE, warning=FALSE}

# Step 1: Calculate Metabolic Weight (Live weight ^ 0.75)
intake_WG_nut_dosage <- intake_WG_nut %>%
  mutate(Metabolic_Weight = `Out.WG.Start_kg` ^ 0.75) %>%
  
  # Step 2: Calculate Feed Dosage (only where applicable)
  mutate(Feed_Dosage_g_DM_per_kg_MW = ifelse(
    `Feed Intake Unit` == "g DM/individual/day",
    `Feed Intake` / Metabolic_Weight,
    NA  # Assign NA for rows that don’t meet the condition
  ))

# View Results
print(intake_WG_nut_dosage)

# Save the full dataset with the new columns
write.csv(intake_WG_nut_dosage, "diet_data/feed_dosage.csv", row.names = FALSE)
```

<<<<<<< HEAD
=======

>>>>>>> 5a1fb4b5e028629b24c515a109644878d64b4a3a
## 4. Plotting simple figures
### 4.1 Plotting weight gain distribution with outliers 
