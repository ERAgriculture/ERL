...
data_variance <- data %>%
  group_by(Code, DOI, Country, Site.ID, CID, C.Descrip, M.Year, Units, MeanC, Tree.Feed, Out.SubInd, PrName, Product.Simple) %>%
  mutate(VarianceT = var(MeanT, na.rm = TRUE)) %>%
  ungroup()
...
head(av_meanT)
write.csv(av_meanT,"diet_data/av_meanT.csv")
