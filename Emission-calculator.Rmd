...
classify_practice_changes <- function(ing_tbl) {
  ...
  ing_tbl %>%
    ...
    map_dfr(function(study) {
      ...
      cmp_block <- function(base_items, comp_items, base_name, comp_name, cmp_flag) {
        ...
        # build two tibbles (added, removed), then stack
        bind_rows(
          tibble(
...
practice_summary <- practice_changes_classified %>%
  ...
  summarise(
    ...
    primary_added = first(D.Item),
    added_types   = paste(unique(D.Type.Class), collapse = "; "),
    .groups = "drop"
  )
