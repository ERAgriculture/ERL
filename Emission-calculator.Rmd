...
## 1.5) Correct D.Type TO BE MODIFIED WHEN PETE UPDATES
...
# library(readxl)
# 
# # ── 1) Canonical lookup from AOM ──────────────────────────────────────────
# aom_url <- "https://raw.githubusercontent.com/Mlolita26/ERA_Docs/main/era_master_sheet(5).xlsx"
# tmp_xls <- tempfile(fileext = ".xlsx")
# download.file(aom_url, tmp_xls, mode = "wb")
# 
# lookup <- read_excel(tmp_xls, sheet = "AOM") %>% 
#   filter(L5 == "Feed Ingredient") %>% 
#   transmute(
#     D.Item     = trimws(Edge_Value),
#     Canon_Type = trimws(L7)
#   ) %>% 
#   filter(!is.na(D.Item), !is.na(Canon_Type)) %>% 
#   distinct()
# 
# # ── 2) Ingredients that carry >1 distinct D.Type ──────────────────────────
# Animals.Diet <- merged_metadata$Animals.Diet   # shorthand
# 
# multi_type_items <- Animals.Diet %>% 
#   group_by(D.Item) %>% 
#   summarise(n_types = n_distinct(D.Type[!is.na(D.Type)]), .groups = "drop") %>% 
#   filter(n_types > 1) %>% 
#   pull(D.Item)                               # *vector* of ambiguous names
# 
# # ── 3) Overwrite only those ambiguous rows when canonical exists ───────────
# Animals.Diet <- Animals.Diet %>% 
#   left_join(lookup, by = "D.Item") %>% 
#   mutate(
#     D.Type = case_when(
#       D.Item %in% multi_type_items & !is.na(Canon_Type) ~ Canon_Type,
#       TRUE                                              ~ D.Type
#     )
#   ) %>% 
#   select(-Canon_Type)
# 
# merged_metadata$Animals.Diet <- Animals.Diet   # write back
# 
# # ── 4) Quick sanity check ──────────────────────────────────────────────────
# fixed_n <- Animals.Diet %>% 
#   filter(D.Item %in% multi_type_items) %>% 
#   distinct(D.Item, D.Type) %>% 
#   add_count(D.Item, name = "n") %>% 
#   filter(n == 1) %>% 
#   summarise(n_fixed = n_distinct(D.Item)) %>% 
#   pull(n_fixed)
# 
# still_bad_n <- Animals.Diet %>% 
#   filter(D.Item %in% multi_type_items) %>% 
#   distinct(D.Item, D.Type) %>% 
#   add_count(D.Item, name = "n") %>% 
#   filter(n > 1) %>% 
#   summarise(n_bad = n_distinct(D.Item)) %>% 
#   pull(n_bad)
# 
# cat("✔  Ingredients corrected :", fixed_n,  "\n",
#     "✖  Still ambiguous       :", still_bad_n, "\n")
