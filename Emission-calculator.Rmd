...
method7 <- function(df){
  ...
  # use the same canonicalization as upstream
  ...
  item_norm,
  regex("\\b(cellulose|micro\\s*-?crystalline\\s+cellulose|carboxy\\s*methyl\\s*cellulose|\\bMCC\\b|\\bCMC\\b)\\b", TRUE))
  out$M7_Category[vi_type_hit | is_shell|is_cellulose] <- "VI"
...
# ─────────────────────────────────────────────────────────────────────────────
# 0) Ensure prerequisites exist (or build them)
#    Assumes you already ran your classification and practice code earlier.
# ─────────────────────────────────────────────────────────────────────────────
method_col <- "M7_Category"

# (A) Per-row INGREDIENT classifications (classif_ing_rows)
if (!exists("classif_ing_rows")) {
  stopifnot(exists("nutr_ing"),
            exists("method1"), exists("method2"), exists("method3"),
            exists("method4"), exists("method5"), exists("method6"), exists("method7"))
  classif_ing_rows <- nutr_ing %>%
    method1() %>% method2() %>% method3() %>% method4() %>%
    method5() %>% method6() %>% method7()
}

# (B) Weight-gain (ADG) standardisation (wg_clean)
if (!exists("wg_clean")) {
  stopifnot(exists("merged_metadata"), exists("weights_unique"))
  ...
  wg <- merged_metadata$Data.Out %>% filter(Out.Subind == "Weight Gain" | is_wg_subind(Out.Subind))
  wg_clean <- wg %>%
    mutate(unit_raw = tolower(str_squish(Out.Unit)),
           value    = suppressWarnings(as.numeric(ED.Mean.T))) %>%
    filter(str_detect(unit_raw, "/d|/day| per day") | str_detect(unit_raw, "g/kg metabolic weight")) %>%
    mutate(
      amount_unit = case_when(str_starts(unit_raw, "mg") ~ "mg",
                              str_starts(unit_raw, "g")  ~ "g",
                              str_starts(unit_raw, "kg") ~ "kg",
                              TRUE ~ NA_character_),
      gain_kg = case_when(amount_unit == "mg" ~ value/1e6,
                          amount_unit == "g"  ~ value/1e3,
                          amount_unit == "kg" ~ value,
                          TRUE ~ NA_real_)
    ) %>%
    left_join(
      weights_unique %>% select(B.Code, A.Level.Name, BW_start_kg = Out.WG.Start),
      by = c("B.Code","A.Level.Name")
    ) %>%
    mutate(
      ADG_kg = case_when(
        str_detect(unit_raw, "g/kg metabolic weight") & !is.na(value) & !is.na(BW_start_kg)
          ~ (value/1000) * (BW_start_kg^0.75),
        TRUE ~ gain_kg
      )
    ) %>%
...
# (C) Daily milk/meat yields (yield_d_clean)
if (!exists("yield_d_clean")) {
  stopifnot(exists("yield_data"))
  MILK_DENSITY <- 1.03
  yield_d_clean <- yield_data %>%
    filter(Out.Subind %in% c("Milk Yield", "Meat Yield")) %>%
    mutate(
    ...
    filter(!is.na(Yield_kg_day) & Yield_kg_day > 0) %>%
    select(B.Code, A.Level.Name, Out.Subind, Yield_kg_day)
}

# (D) Emissions table (GE_combined) must exist
if (!exists("GE_combined")) {
  stop("GE_combined (with CH4_kg_year) is required but not found.")
}

# (E) Practice comparisons table (practice_tbl) must exist (you just built it)
if (!exists("practice_tbl")) {
  stop("practice_tbl is required (built by classify_diet_practice) but not found.")
}

# ─────────────────────────────────────────────────────────────────────────────
# 1) Build per-paper flags for your five conditions
# ─────────────────────────────────────────────────────────────────────────────

# 1) All ingredient rows are classified (by M7)
ing_status <- classif_ing_rows %>%
  group_by(B.Code) %>%
  summarise(
    ing_n               = n(),
...
# 2) Has at least one control–treatment pair and an identified practice
practice_status <- practice_tbl %>%
  group_by(B.Code) %>%
  summarise(
    n_pairs                = n(),
    has_practice_any       = any(!is.na(Practice)),
    has_actionable_practice= any(Practice %in% c("Addition","Substitution")),
    .groups = "drop"
  )
...
adg_status <- wg_clean %>%
  group_by(B.Code) %>%
  summarise(has_adg = any(!is.na(ADG_kg) & ADG_kg > 0), .groups = "drop")

# 4) Has milk or meat yield data
yield_status <- yield_d_clean %>%
  group_by(B.Code) %>%
  summarise(has_yield = any(!is.na(Yield_kg_day) & Yield_kg_day > 0), .groups = "drop")

# 5) Has emission data
emis_status <- GE_combined %>%
  group_by(B.Code) %>%
  summarise(has_emissions = any(!is.na(CH4_kg_year) & CH4_kg_year > 0), .groups = "drop")

# Universe of papers
all_bcodes <- bind_rows(
  ing_status %>% select(B.Code),
  practice_status %>% select(B.Code),
  adg_status %>% select(B.Code),
  yield_status %>% select(B.Code),
  emis_status %>% select(B.Code)
) %>% distinct()

# Merge flags; treat missing as FALSE
paper_flags <- all_bcodes %>%
  left_join(ing_status,      by = "B.Code") %>%
  left_join(practice_status, by = "B.Code") %>%
  left_join(adg_status,      by = "B.Code") %>%
  left_join(yield_status,    by = "B.Code") %>%
  left_join(emis_status,     by = "B.Code") %>%
  mutate(
    ingredients_all_classified = coalesce(ingredients_all_classified, FALSE),
    has_practice_any           = coalesce(has_practice_any, FALSE),
    has_actionable_practice    = coalesce(has_actionable_practice, FALSE),
    has_adg                    = coalesce(has_adg, FALSE),
    has_yield                  = coalesce(has_yield, FALSE),
    has_emissions              = coalesce(has_emissions, FALSE),
    all5_anypractice           = ingredients_all_classified & has_practice_any        & has_adg & has_yield & has_emissions,
    all5_actionable            = ingredients_all_classified & has_actionable_practice & has_adg & has_yield & has_emissions
  )

# ─────────────────────────────────────────────────────────────────────────────
# 2) Counts + handy lists
# ─────────────────────────────────────────────────────────────────────────────
counts_overall <- tibble(
  metric = c(
    "papers_total",
    "ingredients_all_classified",
    "has_practice_any",
    "has_actionable_practice",
    "has_adg",
    "has_yield",
    "has_emissions",
    "ALL FIVE (any practice)",
    "ALL FIVE (actionable only)"
  ),
  n = c(
    nrow(paper_flags),
    sum(paper_flags$ingredients_all_classified),
    sum(paper_flags$has_practice_any),
    sum(paper_flags$has_actionable_practice),
    sum(paper_flags$has_adg),
    sum(paper_flags$has_yield),
    sum(paper_flags$has_emissions),
    sum(paper_flags$all5_anypractice),
    sum(paper_flags$all5_actionable)
  )
)

bcodes_all5_any        <- paper_flags %>% filter(all5_anypractice)  %>% pull(B.Code)
bcodes_all5_actionable <- paper_flags %>% filter(all5_actionable)   %>% pull(B.Code)

# Quick preview (comment out if you’re knitting)
counts_overall
paper_flags %>% arrange(desc(all5_actionable), desc(all5_anypractice))
# bcodes_all5_any
# bcodes_all5_actionable
