...
meta <- merged_metadata$Animals.Diet.Comp   # shorthand

## 0 ── Make ID-type columns characters everywhere -----------------------------
id_cols <- c("Feedipedia", "ilri_code", "Source", "nutrition_source")

# metadata
meta <- merged_metadata$Animals.Diet.Comp %>% 
...
## 1 ── Collapse GE table to one row per key -----------------------------------
#  • remove white-space around D.Item
#  • take the *first* non-missing GE value if duplicates exist
###############################################################################
# 1.  Collapse the separate GE table to a single clean row per key
###############################################################################
ge_keyed <- Gross_energy %>% 
  mutate(
    ## -- housekeeping --------------------------------------------------------
    D.Item            = str_trim(D.Item),
    GE_value          = as.numeric(DC.Value),      # numeric
    nutrition_source  = GE_source                  # <- create / copy here
  ) %>% 
  arrange(B.Code, D.Item, is_entire_diet, desc(!is.na(GE_value))) %>% 
  group_by(B.Code, D.Item, is_entire_diet) %>% 
  slice(1) %>%                                      # keep one representative
  ungroup() %>% 
  transmute(
    B.Code, D.Item, is_entire_diet,
    DC.Variable = "GE",
    ...
    Source,
    Feedipedia,
    nutrition_source,                               # now guaranteed to exist
    ilri_code
  )


## 2 ── Split metadata into GE rows and non-GE rows ----------------------------
meta_ge   <- meta %>% filter(DC.Variable == "GE")
meta_other<- meta %>% filter(DC.Variable != "GE")

## 3 ── (a) FILL missing GE value/unit in rows that already exist --------------
meta_ge_filled <- meta_ge %>% 
  full_join(
    ge_keyed,
    by = c("B.Code", "D.Item", "is_entire_diet", "DC.Variable"),
    suffix = c("", "_new")
  ) %>% 
  mutate(
    ## keep existing value unless it's missing …
    DC.Value = coalesce(DC.Value, DC.Value_new),

    ## … but always adopt the unit from the GE table
    DC.Unit  = coalesce(DC.Unit_new, DC.Unit),   # <-- flip order here!

    ## keep existing provenance unless it was missing
    Source           = coalesce(Source,            Source_new),
    Feedipedia       = coalesce(Feedipedia,        Feedipedia_new),
    nutrition_source = coalesce(nutrition_source,  nutrition_source_new),
    ilri_code        = coalesce(ilri_code,         ilri_code_new)
  ) %>% 
  select(names(meta))    # keep original column order
                # back to original column set/order

## 3 ── (b) ADD GE rows that were totally absent -------------------------------
ge_rows_absent <- ge_keyed %>% 
  anti_join(meta_ge, by = c("B.Code", "D.Item", "is_entire_diet", "DC.Variable")) %>% 
  mutate(
    Source           = coalesce(Source, "filled_from_GE_table"),
    nutrition_source = coalesce(nutrition_source, "filled_from_GE_table")
  ) %>% 
  select(names(meta))

## 4 ── Combine everything ------------------------------------------------------
merged_metadata$Animals.Diet.Comp <- bind_rows(meta_other, meta_ge_filled, ge_rows_absent)

## 5 ── Sanity check ------------------------------------------------------------
merged_metadata$Animals.Diet.Comp %>% 
  filter(DC.Variable == "GE") %>% 
  summarise(n_rows      = n(),
            still_NA    = sum(is.na(DC.Value)),
            .groups = "drop")
...
```{r}
# ============================================================================
#  Ingredient categorisation pipeline with multi‑pass rules & summaries
#  (Ideas 1‒3 integrated)
# ---------------------------------------------------------------------------
#  Requirements: tidyverse 1.3.2 or later
# ============================================================================

margin <- 0.05                       # ±5 % tolerance on numeric thresholds
fmin   <- function(x) x * (1 - margin)
fmax   <- function(x) x * (1 + margin)

# ---- 1.  One‑row‑per‑ingredient averages -----------------------------------
ingredient_avg <- merged_metadata$Animals.Diet.Comp %>%
  filter(is_entire_diet == "FALSE",                       # only single ingredients
         DC.Variable %in% c("NDF", "EE", "GE", "CP")) %>%
...
# ---- 2.  PASS 1 – strict rules (hard priority) -----------------------------
ingredient_pass1 <- ingredient_avg %>%
  mutate(
    Category_code = case_when(
      EE >= 800                                                       ~ "II",  # absolute priority: oils
      NDF <  fmax(50)   & EE >= fmin(800) & CP <  fmax(50)            ~ "II", # fats/oils rule (in case EE < 800)
      between(NDF, fmin(100), fmax(200)) &                            # I – starchy energy feeds
        between(EE,  fmin(20),  fmax(40))  &
        between(CP,  fmin(80),  fmax(120)) &
        between(GE,  fmin(17),  fmax(19))                             ~ "I",
      between(CP,  fmin(350), fmax(500)) &                            # III – protein feeds
        between(NDF, fmin(100), fmax(300))                            ~ "III",
      between(NDF, fmin(400), fmax(500)) &                            # IV – good forages
        between(CP,  fmin(120), fmax(200)) &
        between(EE,  fmin(10),  fmax(30))  &
        between(GE,  fmin(16),  fmax(18))                             ~ "IV",
      between(NDF, fmin(600), fmax(750)) &                            # V – poor roughages
        EE < fmax(20) &
        between(CP,  fmin(30),  fmax(80))  &
        between(GE,  fmin(15),  fmax(17))                             ~ "V",
      TRUE                                                           ~ NA_character_
    )
  )

# ---- 3.  PASS 2 – rescue heuristics (wider net) ----------------------------
ingredient_pass2 <- ingredient_pass1 %>%
  mutate(
    Category_code = coalesce(
      Category_code,
      case_when(
        EE  >= 600                          ~ "II",   # fairly oily
        CP  >= 300 & NDF <= 400             ~ "III",  # clearly protein
        NDF >= 550 & CP <= 100              ~ "V",    # very fibrous
        NDF >= 350 & CP >= 100 & CP <= 250  ~ "IV",   # middling fibre & protein
        TRUE                                ~ NA_character_
      )
    )
  )
...
ingredient_final <- ingredient_pass2 
  

# ---- 5.  Attach descriptive names -----------------------------------------
ingredient_final <- ingredient_final %>%
  mutate(
    Category_name = recode(
      Category_code,
      "I"   = "High-digestible energy feeds (starch & sugars)",
      "II"  = "High-digestible energy feeds (fats/oils)",
      "III" = "High-protein feeds",
      "IV"  = "Good-quality forages & moderate-digestible by-products",
      "V"   = "Poor-quality roughages & fibrous by-products",
      "VI"  = "Mineral/vitamin supplements",
      .default = "Unclassified"
    )
  )

# ---- 6.  Category counts at each stage ------------------------------------
count_cats <- function(df, stage) {
  df %>%
    count(Category_code, name = "n") %>%
    mutate(Stage = stage)
}

summary_counts_long <- bind_rows(
  count_cats(ingredient_pass1, "Pass 1 (strict)"),
  count_cats(ingredient_pass2, "Pass 2 (rescue)")
) %>%
  arrange(Stage, desc(n))

summary_counts_wide <- summary_counts_long %>%
  pivot_wider(names_from = Category_code, values_from = n, values_fill = 0)

# ---- 7.  Output preview ----------------------------------------------------
print(summary_counts_long)
# View(summary_counts_long)  # uncomment inside RStudio
# View(summary_counts_wide)
...
# ────────────────────────────────────────────────────────────────────────────
#  PASS 3 – similarity scoring with coverage‑adjusted confidence
# ────────────────────────────────────────────────────────────────────────────
vars <- c("NDF", "EE", "CP", "GE")           # four nutrients we compare

# 1. Split data --------------------------------------------------------------
classified   <- ingredient_final %>% filter(!is.na(Category_code))
unclassified <- ingredient_final %>% 
  filter(is.na(Category_code)) %>% 
  mutate(row_id = row_number())              # keep row order for later joins

# 2. Z‑score scaling ---------------------------------------------------------
means <- summarise(classified, across(all_of(vars), ~ mean(.x, na.rm = TRUE)))
sds   <- summarise(classified, across(all_of(vars), ~  sd(.x, na.rm = TRUE)))
...
cls_z <- scale_z(classified)
unk_z <- scale_z(unclassified)
...
cent_mat <- as.matrix(centroids[vars])
rownames(cent_mat) <- centroids$Category_code

# 4. Euclidean distances (NA‑safe) ------------------------------------------
dist_pair <- function(u, v) {
  ok <- is.finite(u) & is.finite(v)
  if (!any(ok)) return(Inf)
  sqrt(sum((u[ok] - v[ok])^2) / length(ok))
}

dist_mat <- matrix(NA_real_, nrow = nrow(unk_z), ncol = nrow(cent_mat))
for (j in seq_len(nrow(cent_mat))) {
  dist_mat[, j] <- apply(unk_z[vars], 1, dist_pair, v = cent_mat[j, ])
}
colnames(dist_mat) <- rownames(cent_mat)

# 5. Nearest and second‑nearest centroids ------------------------------------
row_has_data <- apply(is.finite(dist_mat), 1, any)
best_idx     <- rep(NA_integer_, nrow(dist_mat))
second_idx   <- rep(NA_integer_, nrow(dist_mat))

for (i in which(row_has_data)) {
  ord <- order(dist_mat[i, ], na.last = NA)
  best_idx[i]   <- ord[1]
  if (length(ord) >= 2) second_idx[i] <- ord[2]
}

min_dists <- dist_mat[cbind(seq_along(best_idx), best_idx)]
sec_dists <- dist_mat[cbind(seq_along(second_idx), second_idx)]

# 6. Coverage‑adjusted confidence -------------------------------------------
k_available <- rowSums(is.finite(as.matrix(unk_z[vars])))   # 1–4 nutrients
K <- length(vars)

raw_conf <- 1 - min_dists / (min_dists + sec_dists)         # 0–1
adj_conf <- raw_conf * (k_available / K)                    # penalise sparse rows
safety   <- round(100 * adj_conf, 1)                        # % scale

# 7. Assemble Pass 3 results -----------------------------------------------
cat_name <- function(code) recode(
  code,
  "I"   = "High‑digestible energy feeds (starch & sugars)",
  "II"  = "High‑digestible energy feeds (fats/oils)",
  "III" = "High‑protein feeds",
...
unk_scored <- unclassified %>% 
  mutate(
    Closest_category_code = ifelse(is.na(best_idx[row_id]),
                                   NA_character_,
                                   colnames(dist_mat)[best_idx[row_id]]),
    Closest_category_name = cat_name(Closest_category_code),
    Safety_pct            = safety[row_id],
    Stage_assigned        = "Pass 3 (similarity)"
  ) %>% 
  select(-row_id)

# 8. Bring back Pass 1/2 rows and combine ------------------------------------
classified_labeled <- classified %>% 
  mutate(
    Closest_category_code = Category_code,
    Closest_category_name = cat_name(Category_code),
    Safety_pct            = 100,
    Stage_assigned        = "Pass 1/2"
  )

ingredient_final <- bind_rows(classified_labeled, unk_scored)
...
ingredient_final <- ingredient_final %>% 
  ## 0 ── normalise Stage_assigned  ------------------------------------------
  mutate(Stage_assigned = str_replace_all(Stage_assigned, "\u00A0", " ")) %>% 
  ## 1 ── make the decision ---------------------------------------------------
  mutate(
    Safety_pct  = as.numeric(Safety_pct),
    Decision = case_when(
      Stage_assigned == "Pass 1/2"                              ~ "Accepted (rule)",
      Stage_assigned == "Pass 3 (similarity)" & Safety_pct >= 80~ "Accepted (similarity)",
      Stage_assigned == "Pass 3 (similarity)" & Safety_pct >= 60~ "Needs review",
      TRUE                                                      ~ "Unclassified"
    )
  ) %>% 
  ## 2 ── overwrite category only for accepted rows --------------------------
  mutate(
    Category_code = if_else(str_starts(Decision, "Accepted"),
                            Closest_category_code, Category_code),
    Category_name = if_else(str_starts(Decision, "Accepted"),
                            Closest_category_name, Category_name)
  )
...
## helper: flatten → count ---------------------------------------------------
count_cats <- function(df, stage) {
  df %>%
    # 1. flatten any list-column safely
    mutate(
      Category_code = map_chr(
        Category_code,
        ~ dplyr::coalesce(as.character(.x)[1], NA_character_)
      )
    ) %>%
    # 2. keep only rows that really got a code
    filter(!is.na(Category_code)) %>%
    # 3. count and tag stage
    count(Category_code, name = "n") %>%
    mutate(Stage = stage)
}
...
summary_counts_long <- bind_rows(
  ...
  count_cats(
    ingredient_final %>%
      filter(Stage_assigned == "Pass 3 (similarity)",
             Decision       == "Accepted (similarity)"),
    "Pass 3 (similarity)")
) %>%
  arrange(Stage, desc(n))
...
## extra: decision dashboard -------------------------------------------------
decision_summary <- ingredient_final %>%
  count(Decision, name = "n") %>%
  arrange(desc(n))

# ---- 6.  Output preview ---------------------------------------------------
print(summary_counts_long)
print(decision_summary)
# View(summary_counts_long)   # uncomment in RStudio
# View(summary_counts_wide)
...
# ── 1. List rows that still need attention ──────────────────────────────────
review_candidates <- function(df,
                              columns = c("D.Item",
                                          "Decision",
                                          "Closest_category_name",
                                          "Safety_pct",
                                          "Closest_category_code")) {
  df %>%
    filter(Decision %in% c("Needs review", "Unclassified")) %>%
    select(all_of(columns))
}

# ── 2. Promote selected items to "Accepted (manual)" ────────────────────────
#    accepted_vec = character vector of D.Item values you’ve approved
accept_reviewed <- function(df, accepted_vec) {
  df %>%
    mutate(
      Decision = case_when(
        D.Item %in% accepted_vec &
          Decision %in% c("Needs review", "Unclassified")  ~ "Accepted (manual)",
        TRUE                                               ~ Decision
      ),
      Category_code = if_else(D.Item %in% accepted_vec &
                                Decision == "Accepted (manual)",
                              Closest_category_code, Category_code),
      Category_name = if_else(D.Item %in% accepted_vec &
                                Decision == "Accepted (manual)",
                              Closest_category_name, Category_name)
    )
}
...
```{r}
## 1. Pull the list that still needs a decision
to_check <- review_candidates(ingredient_final)

## 2. Suppose you manually approve three items
approved_items <- c("Fish Meal",
                    "Clupeid Fish Meal",
                    "Fish Meal Pelleted",
                    "Fish Meal Roasted",
                    "Maize Offal",
                    "Molasses",
                    "Maize Meal",
                    "Soybean")   # ← your choices

## 3. Flip their status to Accepted (manual)
ingredient_final <- accept_reviewed(ingredient_final, approved_items)

## 4. Re-run any summary/count blocks if you want updated tallies

```

```{r}
# --------------------------------------------------------------------------
# 9.  OVERWRITE D.Type WITH NUTRITIONAL CATEGORY (if accepted)
# --------------------------------------------------------------------------
# assume:
#   ingredient_final  – result of Pass 4 (has Category_name, Decision, D.Item)
#   ingredients       – your original table (columns shown in the paste)
...
ingredient_final <- ingredient_final %>%
  mutate(
    # ----------------------------------------------------------------------
    # 1. Force a Category_code when the product name matches
    # ----------------------------------------------------------------------
    Category_code = case_when(
      # High-protein feeds (III)
      D.Item %in% c(
        "Noug Cake", "Cotton Seed Cake", "Cotton Seed Meal",
        "Sunflower Seed Meal", "Sunflower Seed Cake", "Soybean Cake",
        "Canola Meal", "Fish", "Fish Meal", "Fish Meal Pelleted",
        "Fish Meal Roasted"
      )                                                                  ~ "III",
      
      # Good-quality forages & by-products (IV)
      D.Item %in% c(
        "Brewers Grain Dried", "Brewers Grain", "Wheat Offal",
        "Rice Offal", "Leucaena leucocephala Leaves Dried",
        "Moringa oleifera Leaves Dried Ground"
      )                                                                  ~ "IV",
      
      # Poor-quality roughages (V)
      D.Item %in% c("Eragrostis curvula Hay", "Cellulose")               ~ "V",
      
      # Energy feeds – fats/oils (II)
      D.Item %in% c("Soybean Oil", "Sunflower Oil")                      ~ "II",
      
      # Energy feeds – starch/sugars (I)
      D.Item %in% c("Cassava Peel", "Sorghum Seed","Unspecified Molasses")                      ~ "I",
      
      # Mineral/vitamin supplements (VI)
      D.Item %in% c("Bone", "Bone Meal")                                 ~ "VI",
      
      TRUE                                                               ~ Category_code
    ),
    
    # ----------------------------------------------------------------------
    # 2. Mark these rows as simply "Accepted"
    # ----------------------------------------------------------------------
    Decision = if_else(
      D.Item %in% c(
        "Noug Cake", "Cotton Seed Cake", "Cotton Seed Meal",
        "Sunflower Seed Meal", "Sunflower Seed Cake", "Soybean Cake",
        "Canola Meal", "Fish", "Fish Meal", "Fish Meal Pelleted",
        "Fish Meal Roasted", "Brewers Grain Dried", "Brewers Grain",
        "Wheat Offal", "Rice Offal", "Leucaena leucocephala Leaves Dried",
        "Moringa oleifera Leaves Dried Ground", "Eragrostis curvula Hay",
        "Cellulose", "Soybean Oil", "Sunflower Oil", "Cassava Peel",
        "Sorghum Seed", "Bone", "Bone Meal"
      ),
      "Accepted(Manual)",                           # ← new value
      Decision
    ),
    
    # ----------------------------------------------------------------------
    # 3. Refresh the readable label to match the code
    # ----------------------------------------------------------------------
    Category_name = recode(
      Category_code,
      "I"   = "High-digestible energy feeds (starch & sugars)",
...
# 1. Pull the accepted category label for each D.Item -----------------------
accepted_labels <- ingredient_final %>%
  filter(startsWith(Decision, "Accepted")) %>%   # rule or similarity
  select(D.Item, New_D.Type = Category_name) %>%
  distinct()

# 2. Merge & overwrite D.Type where a new label exists ----------------------
ingredients <- ingredients %>%
  left_join(accepted_labels, by = "D.Item") %>%         # adds New_D.Type
  rename(D.Type_Nutri = New_D.Type)                     # keep both columns
        # replace only when we have one
                          # drop helper column
...
```{r}
# ---------------------------------------------------------------------------
#  AFTER you have ingredients with `D.Type_Nutri`  (may still contain NA)
# ---------------------------------------------------------------------------
nonenergy_types <- c("Non-ERA Feed", "Supplement")   # add more if needed

ingredients <- ingredients %>%
  mutate(
    D.Type_Nutri = coalesce(
      D.Type_Nutri,                                                # keep any prior assignment
      case_when(
        D.Type %in% nonenergy_types                                ~ "Mineral/vitamin supplements",  # VI
        str_detect(D.Item, regex("^salt$", ignore_case = TRUE))    ~ "Mineral/vitamin supplements",  # VI
        TRUE                                                       ~ NA_character_
      )
    )
  )
...
unclassified_products <- ingredients %>%          # table that has D.Type_Nutri
  filter(is.na(D.Type_Nutri)) %>%                 # still no nutritional class
  group_by(D.Item) %>%                            # one block per product
  summarise(
    rows          = n(),                          # how many rows total
    unique_codes  = n_distinct(B.Code)            # how many different B.Code
  ) %>%
  mutate(
    perc = round(100 * rows / sum(rows), 2)       # share of *all* unclassified
  ) %>%
  arrange(desc(rows))                             # most frequent first

print(unclassified_products, n = 20)   # show top 20; tweak n as needed
...
quantify_practice_changes <- function(ingredients_clean, min_pct = 0.5) {
  ...
  result <- ing[, {
    ...
    dt <- dt[, .(
      D.Type_Nutri          = first(D.Type_Nutri),          # keep the category
    ...
    if (length(pairs) == 0) {
      ...
      rbindlist(lapply(pairs, function(p) {
        ...
        grid <- merge(
          dA[, .(D.Item, D.Type_Nutri, pct_A = pct_diet,
                   unit_A = paste(D.Unit.Amount, D.Unit.Time, D.Unit.Animals))],
          dB[, .(D.Item, D.Type_Nutri, pct_B = pct_diet,
                   unit_B = paste(D.Unit.Amount, D.Unit.Time, D.Unit.Animals))],
          by = c("D.Item", "D.Type_Nutri"), all = TRUE, allow.cartesian = FALSE
        )
        ...
        fmt <- function(dt) {
          ...
          paste(paste0(dt$D.Type_Nutri, ":", round(dt$V1,2)), collapse = "; ")
        }

        add_by_type  <- fmt(grid[pct_A == 0 & delta  > 0,  .(V1 = sum(delta)), by = D.Type_Nutri])
        rem_by_type  <- fmt(grid[pct_B == 0 & delta  < 0,  .(V1 = -sum(delta)), by = D.Type_Nutri])
        inc_by_type  <- fmt(grid[pct_A > 0 & pct_B > 0 & delta > 0,  .(V1 = sum(delta)), by = D.Type_Nutri])
        dec_by_type  <- fmt(grid[pct_A > 0 & pct_B > 0 & delta < 0,  .(V1 = -sum(delta)), by = D.Type_Nutri])
...
explode <- function(.data, col, kind) {                         # { 1
  .data %>% 
    ...
    separate(.data[[col]], into = c("D.Type_Nutri", "pct"),           # "A:10" → A | 10
             sep = ":", convert = TRUE, fill = "right") %>% 
    mutate(
      # ─ collapse categories ────────────────────────────────────────────
      D.Type_Nutri = case_when(
        str_detect(D.Type_Nutri, regex("supplement",  ignore_case = TRUE)) ~ "Supplement",
        str_detect(D.Type_Nutri, regex("concentrate", ignore_case = TRUE)) ~ "Concentrate",
        TRUE                                                        ~ D.Type_Nutri
      ),
      change_kind = kind
    ) %>% 
    filter(D.Type_Nutri != "Entire Diet")                               # drop whole-diet rows
}                                                                 # } 1
