...
csv_path <- "Ingredients_nutrition_unmatched_GPT.csv"  # <- change if saved elsewhere
...
# 0) Keys for unmatched diets (canonicalize to be safe)
...
diet_perc_unmatched <- diet_percentages %>%
  mutate(
    D.Item       = canonicalize_item(D.Item),        # ingredient
    A.Level.Name = canonicalize_item(A.Level.Name),  # diet/treatment
...
diet_perc_nutr <- diet_perc_unmatched %>%
  left_join(canon_ing %>% select(D.Item, GE, Ash, CP, EE, NDF), by = "D.Item")
